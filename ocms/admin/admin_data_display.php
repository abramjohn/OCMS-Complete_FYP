...
if(isset($_POST['delete_admin']))
	{
		$key = $_POST['delete_admin_key'];
		$query = "SELECT *  FROM admin_register WHERE admin_id='{$key}'";
        $result=mysqli_query($connection,$query);
        if(mysqli_num_rows($result)>0)
		{
			$query_delete = "DELETE FROM admin_register WHERE admin_id='{$key}'";
			$result_delete=mysqli_query($connection,$query_delete);
			
			
			header("Location:admin_data_display.php");
		}
		else
		{}
...
$t=mysqli_query($connection,"SELECT *  FROM admin_register");
...
<div class="container">
	...
	<table class="table table-bordered" align="center" style="background:rgba(0,0,0,0.7);">
		...
		<?php
		$sr = 1;
		$query = "SELECT *  FROM admin_register ORDER BY admin_id DESC LIMIT $start,$limit";
        $result=mysqli_query($connection,$query);
		while($row = mysqli_fetch_array($result)){
			$admin_id = $row['admin_id'];
		?>
		<tr>
			<form method="post" action="admin_data_display.php">
				<td style="color: white;"><?php echo $sr;?></td>
				<td style="color: white;"><?php echo $row['name'];?></td>
				<td style="color: white;"><?php echo $row['email'];?></td>
				<td style="color: white;"><?php echo $row['contact'];?></td>
				<td style="color: white;"><?php echo $row['designation'];?></td>
		...
		if(isset($_SESSION['admin_id']))
		{
			if($_SESSION['admin_checker']==1)
					...
					{
                            echo "
				<td><input type='checkbox' value='$admin_id' name='delete_admin_key' required></td>
				<td><input class='btn btn-danger btn-sm' type='submit' name='delete_admin' value='Delete'></td>";
