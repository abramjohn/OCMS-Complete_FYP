...
$t=mysqli_query($connection,"SELECT *  FROM cars");
...
<div class="container">
	...
	<table class="table table-bordered" align="center" style="background:rgba(0,0,0,0.7);">
		...
		<?php
		$sr = 1;
		$query = "SELECT *  FROM cars ORDER BY car_id DESC LIMIT $start,$limit";
        $result=mysqli_query($connection,$query);
		...
		<tr>
			<form method="post" action="delete_function.php">
				...
				<td style="color: white;"><?php echo $row['car_name'];?></td>
				<td style="color: white;"><?php echo $row['car_model'];?></td>
				<td style="color: white;"><?php echo $row['veh_type'];?></td>
				<td><input type='checkbox' value="<?php echo $row['car_id'];?>" name='delete_car_key' required></td>
				<td><input class='btn btn-danger btn-sm' type='submit' name='delete_car' value='Delete'></td>
