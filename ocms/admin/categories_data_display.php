...
$t=mysqli_query($connection,"SELECT *  FROM parts_categories");
...
<div class="container">
	...
	<table class="table table-bordered" align="center" style="background:rgba(0,0,0,0.7);">
		...
		<?php
		$sr = 1;
		$query = "SELECT *  FROM parts_categories ORDER BY prt_id DESC LIMIT $start,$limit";
        $result=mysqli_query($connection,$query);
		...
		<tr>
			<form method="post" action="delete_function.php">
				...
				<td style="color: white;"><?php echo $row['prt_name'];?></td>
				<td><input type='checkbox' value="<?php echo $row['prt_id'];?>" name='delete_prt_key' required></td>
				<td><input class='btn btn-danger btn-sm' type='submit' name='delete_prt' value='Delete'></td>
